...
fn frozen_both_axes_add_separator_on_each() {
    ...
    let frame = Chrome::next(None, &m, test_canvas(), &std::rc::Rc::new(LIGHT), FramePath::Fresh);
    let p = &frame.pane_set;
...
fn rebuild_recycles_pane_slot_buffers() {
    ...
    let f1 = Chrome::next(None, &m, test_canvas(), &std::rc::Rc::new(LIGHT), FramePath::Fresh);
    ...
    let f2 = Chrome::next(Some(f1), &m, test_canvas(), &std::rc::Rc::new(LIGHT), FramePath::Fresh);
