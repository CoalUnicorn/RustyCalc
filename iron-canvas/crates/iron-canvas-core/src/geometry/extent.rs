//! Canvas extent: the drawable size and its parsed DPR pair.
//!
//! [`CanvasSize`] is the permissive logical (CSS) size; [`CanvasMetrics`] is
//! the size/DPR pair parsed once at the host boundary, with
//! [`CanvasMetricError`] as its parse failure. Every geometry walk, hit test,
//! backend, and export path names them.
