...
//! Diagnostics and `GridCache`'s painted-fingerprint gating
//! (via `reuses_slots()`) read it; orchestrator `paint_*`
...
impl FrameKindTag {
    ...
    /// `SlotsReused` and `Blitted` carry forward compatible grid cache state.
