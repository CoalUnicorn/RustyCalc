...
Holds rows, typed columns, and per-cell values/styles in memory, and implements `iron-canvas-core`'s `CanvasModel` so the renderer can paint it directly. No formulas, no recalculation, no IronCalc — just "here is a table, draw it." Supports column sizing/alignment and in-place sorting.
...
```rust
let grid = DataGridBuilder::new()
    .column("Name", 160, HAlign::Left)
    .column("Score", 80, HAlign::Right)
    .row(vec!["Ada", "99"])
    .build();
