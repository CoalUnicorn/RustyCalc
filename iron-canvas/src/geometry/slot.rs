...
pub(crate) trait AxisSlot: Sized {
...
pub(crate) fn fill_axis<S: AxisSlot>(
    slots: &mut Vec<S>,
...
#[inline]
pub(crate) fn scroll_first(frozen_count: i32, view_first: i32) -> i32 {
...
pub(crate) fn top_id<S: AxisSlot>(scroll: &[S]) -> i32 {
...
pub(crate) fn last_visible_id<S: AxisSlot>(scroll: &[S]) -> i32 {
    scroll
        .last()
        .map(|s| s.id())
...
pub(crate) fn pixel_to_id<S: AxisSlot>(frozen: &[S], scroll: &[S], pixel: i32) -> Option<i32> {
...
pub(crate) fn boundary_at<S: AxisSlot>(
    frozen: &[S],
    scroll: &[S],
    pixel: i32,
    hit_zone: i32,
