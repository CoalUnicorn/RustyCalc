...
impl CanvasRenderer {
    ...
    fn render_cell_style(
        ...
        let left_nb = if col > 1 && style.border.left.is_none() && style.fill.fg_color.is_none() {
        ...
        let (bl_color, bl_style) = if let Some(ref bl) = style.border.left {
            ...
            let color = if style.fill.fg_color.is_some() {
                bg
            } else {
                cell_grid_color
            };
    ...
    fn draw_selection(&self, model: &UserModel, sheet: u32, frozen: FrozenOffset) {
        ...
        // Autofill handle: solid 6x6 square at bottom-right corner of range
